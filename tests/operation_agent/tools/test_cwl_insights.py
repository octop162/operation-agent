...
def test_cwl_insights_tool_exists():
    from operation_agent.tools.cwl_insights import cwl_insights  # noqa: F401


def test_cwl_insights_is_strands_tool():
    from strands.tools.decorator import DecoratedFunctionTool

    from operation_agent.tools.cwl_insights import cwl_insights
...
def test_cwl_insights_returns_string_on_success():
    ...
    with patch("operation_agent.tools.cwl_insights.boto3.client") as mock_boto:
...
def test_cwl_insights_polls_until_complete():
    ...
    with (
        patch("operation_agent.tools.cwl_insights.boto3.client") as mock_boto,
        patch("operation_agent.tools.cwl_insights.time.sleep") as mock_sleep,
    ):
        mock_client = MagicMock()
        mock_boto.return_value = mock_client
...
def test_cwl_insights_raises_on_query_failed():
    with (
        patch("operation_agent.tools.cwl_insights.boto3.client") as mock_boto,
        patch("operation_agent.tools.cwl_insights.time.sleep"),
    ):
        mock_client = MagicMock()
        mock_boto.return_value = mock_client
...
def test_cwl_insights_raises_on_timeout():
    with (
        patch("operation_agent.tools.cwl_insights.boto3.client") as mock_boto,
        patch("operation_agent.tools.cwl_insights.time.sleep"),
        patch("operation_agent.tools.cwl_insights._POLL_MAX_ATTEMPTS", 3),
    ):
        mock_client = MagicMock()
        mock_boto.return_value = mock_client
...
def test_cwl_insights_uses_region_from_config(monkeypatch):
    monkeypatch.setenv("AGENT_AWS_REGION", "us-west-2")
    results = [[{"field": "@message", "value": "ok"}]]
